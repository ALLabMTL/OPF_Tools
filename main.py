...
for instance in cases:
    for relaxation in relaxations:
        try:
            case = loadCase('cases/'+instance+'.json')
            ans = runOPF(case, relaxation, Verbose, solver)
        except:
            print('Error loading case {} with relaxation {}'.format(instance, relaxation))
            errors.append((instance, relaxation))
            continue
        print('Case: {}, Relaxation: {}'.format(instance, relaxation))
        print('Optimal result is : {:.3f}'.format(ans.loss))
        print('Solve time is : {:.2f} seconds'.format(ans.solve_time))
        print('Compilation time is : {:.2f} seconds'.format(ans.compilation_time))
        print()